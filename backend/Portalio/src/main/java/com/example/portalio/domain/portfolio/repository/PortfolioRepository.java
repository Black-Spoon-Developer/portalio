...
public interface PortfolioRepository extends JpaRepository<Portfolio, Long> {
    ...
    @Query("SELECT p FROM Portfolio p LEFT JOIN PortfolioComment c ON p.portfolioId = c.portfolio.portfolioId GROUP BY p.portfolioId, p.portfolioViews, p.portfolioRecommendationCount ORDER BY (p.portfolioViews + p.portfolioRecommendationCount + COUNT(c)) DESC")
